...
class JYACC:
    ...
    def writeShow(name,args):
        ...
        for n in nodes:
            lines += [JYACC._WRITELINE(2,"_PRINTLINE(offset + 1,\"" + n[1] + ":\")"),JYACC._WRITELINE(2,"if self."+n[1] + " is not None:")]
            if n[0] == "n":
...
with open("SYNTAX_TO_AST\\JNode2.cfg") as F:
    with open("SYNTAX_TO_AST\\JTree.py",'w') as O:
